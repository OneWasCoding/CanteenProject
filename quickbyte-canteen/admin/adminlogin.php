...
$email = 'ad123min@gmail.com'; // Fixed email address
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($password)) {
        ...
        // Validate User
        // Check if the user exists AND has the Admin role AND verify their password
        ...
        if ($result->num_rows > 0) {
            $found = false; // Flag to check if matching password
            ...
            while ($admin = $result->fetch_assoc()) {
                ...
                if (password_verify($password, $admin['password'])) {
                    ...
                    $_SESSION['user_name'] = $admin['name']; // Store admin's name
                    ...
                    exit();
                  $found = true;
                  break; // break the loop since it logged in!
                }
            }

            if (!$found) {
                // Increment failed attempts for first admin with the username
                $admin_check_sql = "SELECT * FROM users WHERE email = ? AND role = 'Admin'";
                $stmt = $con->prepare($admin_check_sql);
                $stmt->bind_param("s", $email);
                $stmt->execute();
            ...
            $error = "Account is not authorized to access admin!";
        }
    }
}
?>
...
<head>
    ...
    <style>
        /* All previous CSS from the original file remains UNCHANGED */
        body {
        ...
        <div class="login-container">
            ...
            <div class="form-group">
                    ...
                    <input type="email" class="form-control" id="email" name="email"
                           placeholder="Enter your email" value="ad123min@gmail.com" required readonly>
                </div>
                ...
                <div class="form-group">
                    ...
                    <?php if (isset($admin['failed_attempts']) && $admin['failed_attempts'] > 0): ?>
                        <div class="attempts-warning">
                            <?php echo (3 - $admin['failed_attempts']); ?> attempts remaining
                        </div>
                    <?php endif; ?>
                </div>
