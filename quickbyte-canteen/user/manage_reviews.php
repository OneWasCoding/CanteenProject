...
// 1) Fetch all reviews made by the user (without item_id)
$sql_reviews = "
...
// 2) If the user has no reviews, fetch completed orders (by stall) that haven’t been reviewed
$notReviewedOrders = [];
if (!$hasReviews) {
    // Here we assume an order is "not reviewed" if it's completed and
    // there's no feedback record for that stall_id + user_id pair.
    $sql_notReviewed = "
        SELECT o.order_id, o.order_date, o.total_price, s.stall_id, s.stall_name
        FROM orders o
...
<head>
    ...
    <style>
        ...
        .review-rating { color: #ffcc00; }
        .btn-group .btn { margin-right: 0.5rem; }
        .orders-list { margin-top: 2rem; }
        footer {
            background: linear-gradient(135deg, #e44d26, #ff7f50);
            color: white;
            text-align: center;
    ...
    <div class="container mt-5">
        <div class="reviews-container">
            ...
            <?php if (!empty($reviews)): ?>
                <!-- Display existing reviews with Edit and Delete buttons -->
                <?php foreach ($reviews as $review): ?>
                    <div class="review-card">
                        <div class="review-header">
                            <h4><?php echo htmlspecialchars($review['stall_name']); ?></h4>
                        </div>
